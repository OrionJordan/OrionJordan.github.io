...
We first examine the language of first-order logic; which defines the list of symbols used in the system, and the rules for how we can combine them into formulas.
...
A [bound]{.definition} variable of some formula $\phi$ is a variable occurring in the scope of a quantifier.
:::
...
the variable $x$ is bound, since it occurs directly after the existential quantifier. The variable $y$ however is free, as it is not quantified over. This gives us our next definition.

::: {.callout-note icon="false"}
## Definition 

A [free]{.definition} variable of some formula $\phi$ is a variable which is not bound.
:::

We now know what symbols are used in our language, how we can combine them into formulas, and the differences between bound and free variables. We now move onto the deductive system of first-order logic.
