...
Continuity is a local condition on a function. It is defined at each point $c \in D$, and cares only about an infinitesimally small neighborhood about said point. Uniform continuity is a stronger notion, defined over the entirety of the domain $D$ of a function.
...
A function $f \colon D \to \mathbb{R}$ is [uniformly continuous]{.definition} on $D$ if for all $\varepsilon > 0$, there exists $\delta > 0$ such that for all $x,y \in D$, $|x-y| < \delta \implies |f(x) - f(y)| < \varepsilon$.
:::
...
Let $f\colon D \to R$ be a uniformly continuous function on D. Then $f$ is continuous on $D$.
...
If $f$ is uniformly continuous on $D$, then for all $\varepsilon$ there exists $\delta$ such that for all $x,y \in D$

$$
|x - y| < \delta \implies |f(x) - f(y)| < \varepsilon.
$$

Then for any $c \in D$, we have

$$
|x - c| < \delta \implies |f(x) - f(c)| < \varepsilon
$$

so $f$ is continuous at $c$ for all $c \in D$, and thus $f$ is continuous on $D$.
:::
...
This definition allows us to take a function $f$, and create a new function $\tilde{f}$ such that $f(x) = \tilde{f}(x)$ for all $x \in D$. If we extend $f$ to a domain, and prove $\tilde{f}$ is uniformly continuous on said domain, then clearly $f$ is uniformly continuous on it's sub-domain.
