...
A [**sequence**]{.definition} $S$ is a function whose domain is the set of natural numbers, $\mathbb{N}$.
:::
...
A sequence $(s_n)$ of real numbers [**converges**]{.definition} to a real number $s$ if
...
By the Archimedean property, we know there always exists some $N_\varepsilon \in \mathbb{N}$ such that $0 < \frac{1}{N_\varepsilon} < \varepsilon$.
...
The above theorem may not seem useful now, but will come in handy in the future. 
...
Let $M = \sup\{|s_1|,|s_2|,\ldots,|s_N|,|s| + 1\}$. Thus for all $n \in \mathbb{N}$, $|s_n| \leq M$ and $(s_n)$ is bounded.
:::
